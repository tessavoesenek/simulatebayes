...
make_interventions <- function(data = participants, Effect = -0.04, Kwartaaleffect = -0.01, Runs = 20){
  ...
  for(i in 1:Runs){
    ...
    df <- data %>%
        mutate(K2 = rnorm(n = n_participants, mean = (1 + Kwartaaleffect) * K1, sd = 0.02),
      ...
      mutate(Kwartaal = stringr::str_sub(Kwartaal, start = 2)) %>%
